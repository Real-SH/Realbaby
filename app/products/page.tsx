      ...
      <main className="inner-page">
        <section className="inner-hero product-index-hero">
          <div className="container inner-hero-grid">
            <div><p className="section-kicker">RealBaby Product Collection</p><h1>Designed around how childhood really happens.</h1><p className="inner-lead">Explore fifteen developed directions across first play, hands-on learning, and everyday comfort. Each can be adapted for your market, sales channel, price direction, and brand.</p><small className="scope-note">Series ages and product ages are planning directions only. Final age grading is confirmed for the actual construction, intended use, testing plan, and destination market.</small><div className="cta-row about-actions"><Link className="btn btn-primary" href="/contact">Request a Proposal</Link><a className="btn btn-outline" href="/realbaby-product-catalog.pdf">Download 2026 Catalog</a></div></div>
            <div className="fact-panel"><strong>3 focused series · 15 directions</strong><span>Typical MOQ: 500 pcs+</span><span>Typical sampling: 10-15 days</span><span>Typical production: 40-70 days</span><span>Project-specific testing & compliance support</span><small>Reference values only. Final terms are confirmed for the actual product and project.</small></div>
          </div>
        </section>
          ...
          return (
            <section className="section product-series-block" id={series.name.toLowerCase().replaceAll(" ", "-")} key={series.name}>
              <div className="container">
                <div className="product-series-heading"><div><p className="section-kicker">Series direction · {series.age}</p><h2>{series.name}</h2></div><p>{series.description}</p></div>
                <div className="product-catalog-grid">
                  {seriesProducts.map((product) => (
                    <article className="catalog-card" key={product.slug}>
                      <Link className="catalog-card-image" href={`/products/${product.slug}`}><Image src={product.image} alt={product.imageAlt} width={900} height={1200} sizes="(max-width: 620px) 100vw, (max-width: 960px) 50vw, 33vw" /><span className="age-badge" title="Planning direction; final age grade is project-specific">{product.age} direction</span></Link>
